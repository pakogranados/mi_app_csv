...
def new_production():
    ...
    cursor.execute("SELECT id, nombre FROM mercancia")   # si luego defines tabla finished_products, cámbialo
...
def production_materials(production_id):
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            INSERT INTO production_detalle_mp (production_id, mercancia_id, cantidad_usada)
            VALUES (%s, %s, %s)
    ...
    cursor.execute("""
        SELECT pd.id, m.nombre, pd.cantidad_usada AS quantity_used
        FROM produccion_detalle_mp pd
        JOIN mercancia m ON pd.mercancia_id = m.id
...
def delete_production(production_id):
    ...
    # ⚠️ Opción simple: borrar la orden.
    # Si tu tabla production_wip tiene FK ON DELETE CASCADE, sus filas se borran solas.
    # OJO: si generas inventario_movimientos al meter WIP, esto NO los revierte.
    cursor.execute("DELETE FROM production WHERE id = %s", (production_id,))
    conn.commit()

    cursor.close(); conn.close()
    flash('Orden de producción eliminada.', 'success')
    return redirect(url_for('list_production'))
...
def production_wip(production_id):
    ...
    if request.method == 'POST':
        ...
        # 🔹 Buscar último precio unitario del insumo
        cursor.execute("""
        ...
        """, (product_id,))
        row = cursor.fetchone()
        unit_cost = float(row['precio_unitario']) if row and row['precio_unitario'] else 0
        ...
        # Registrar SALIDA de inventario de materia prima
        cursor.execute("""
            INSERT INTO inventario_movimientos
            (tipo_inventario_id, mercancia_id, tipo_movimiento, unidades, precio_unitario, referencia, fecha)
            VALUES (2, %s, 'SALIDA', %s, %s, %s, NOW())
        """, (product_id, quantity, unit_cost, f'Producción {production_id} - Consumo WIP'))

        conn.commit()
        flash('Materia prima consumida y registrada en WIP.', 'success')
        cursor.close(); conn.close()
...
def recetas():
    ...
    if request.method == 'POST':
        ...
        inserts = 0
        for mid, qty, uni in zip(material_ids, quantities, units):
            if not mid or not qty:
                continue
            cursor.execute(
                "INSERT INTO recipes (product_id, material_id, quantity, unit) VALUES (%s, %s, %s, %s)",
                (product_id, mid, qty, uni)
            )
            inserts += 1
        conn.commit()
        flash(f"Receta guardada ({inserts} insumos).", "success")
        return redirect(url_for('recetas'))

    # Productos terminados
    cursor.execute("SELECT id, nombre FROM mercancia WHERE tipo='PT'")
    productos = cursor.fetchall()

    # Materias primas
    cursor.execute("SELECT id, nombre FROM mercancia WHERE tipo='MP'")
    materiales = cursor.fetchall()

    # Recetas registradas (vista)
    cursor.execute("""
        SELECT r.id, p.nombre AS producto, m.nombre AS material, r.quantity, r.unit
        FROM recipes r
        JOIN mercancia p ON r.product_id = p.id
        JOIN mercancia m ON r.material_id = m.id
        ORDER BY p.nombre
    """)
    ...
    return render_template("inventarios/PT/recetas.html", 
                       productos=productos, 
...
def productos_terminados():
    if request.method == "POST":
        ...
        # Guardamos en mercancia con tipo = 'PT'
        cursor.execute("""
            INSERT INTO mercancia (nombre, tipo)
            VALUES (%s, %s)
        """, (nombre, "PT"))
        conn.commit()
        ...
        cursor.close()
        conn.close()
        flash("Producto Terminado agregado correctamente", "success")
        return redirect(url_for("productos_terminados"))
...
def panel_de_control():
    ...
    # Terminados vía movimientos tipo_inventario_id=3
    cur.execute("""
        SELECT
          COALESCE(SUM(CASE WHEN UPPER(tipo_movimiento)='ENTRADA' THEN unidades ELSE 0 END)
               - SUM(CASE WHEN UPPER(tipo_movimiento)='SALIDA'  THEN unidades ELSE 0 END), 0) AS terminados
        FROM inventario_movimientos
...
def nueva_compra():
    ...
    if request.method == 'POST':
        ...
        for prod in productos:
            cursor.execute("SELECT 1 FROM presentaciones WHERE descripcion = %s", (prod.strip(),))
            if not cursor.fetchone():
                cursor.close(); conn.close()
                flash(f"La presentación '{prod.strip()}' no está registrada.", "danger")
        ...
        try:
            ...
            if metodo_pago == 'credito':
                        cursor.execute("""
                            INSERT INTO compras_credito (compra_id, fecha, numero_documento, proveedor, importe, iva, ieps, total)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                        """, (compra_id, fecha, numero_factura, proveedor, float(total_general), 0.0, 0.0, total_general ))
            ...
            for prod, und, pu, pt in zip(productos, unidades, precios, totales):
                ...
                # 🔁 AHORA buscamos por descripción de PRESENTACIÓN
                cursor.execute("""
                    SELECT m.id AS mercancia_id,
                        COALESCE(p.contenido_neto, m.cont_neto) AS cont_neto
                    FROM presentaciones p
                    JOIN mercancia m ON m.id = p.mercancia_id
                    WHERE p.descripcion = %s
                    LIMIT 1
                """, (prod.strip(),))
                row = cursor.fetchone()
                if not row:
                    # si no existe la presentación, salta o maneja el error
                    flash(f"La presentación '{prod.strip()}' no existe.", "danger")
                    continue

                mercancia_id = row["mercancia_id"]
                try:
                    cont_neto = float(row["cont_neto"] or 1)
                    if cont_neto <= 0:
                        cont_neto = 1
                ...
                contenido_neto_total = unidades_base * cont_neto
                ...
                """, (
                    compra_id,
                    mercancia_id,
                    prod.strip(),             # aquí queda la descripción de la presentación
                    unidades_base,
    ...
    cursor.execute("SELECT descripcion FROM presentaciones ORDER BY descripcion")
    productos = [row['descripcion'] for row in cursor.fetchall()]
    cursor.close(); conn.close()
...
def mercancia():
    ...
    if request.method == 'POST':
        nombre = request.form['nombre'].strip()
        cont_neto = (request.form.get('cont_neto') or '0').strip()
        ...
        try:
            ...
            conn.commit()
            flash(f'Producto registrado. Subcuenta: {subcuenta_codigo}', 'success')
        except Exception as e:
            conn.rollback()
    ...
    try:
        ...
        cur.execute("""
            UPDATE mercancia
             ...
             WHERE id=%s
        """, (nombre, str(cont_neto), unidad_id, iva, ieps, subcuenta_id, cuenta_padre_id, id))
        ...
        conn.commit()
        flash('Mercancía actualizada.', 'success')
    except Exception as e:
        conn.rollback()
...
def eliminar_mercancia(id):
    ...
    try:
        ...
        cur.execute("DELETE FROM production_detalle_mp WHERE mercancia_id=%s", (id,))
        cur.execute("DELETE FROM production_wip WHERE product_id=%s", (id,))
        cur.execute("DELETE FROM recipes WHERE material_id=%s OR product_id=%s", (id,id))
        cur.execute("DELETE FROM mercancia WHERE id=%s", (id,))
        conn.commit(); flash('Producto eliminado.', 'success')
    except Exception as e:
        conn.rollback(); flash(f'No se pudo eliminar: {e}', 'danger')
    finally:
        cur.close(); conn.close()
    return redirect(url_for('mercancia'))
