...
def inventario_movimientos(mercancia_id):
    ...
    for m in movimientos:
        ...
        # Unidades
        entrada_u = float(
            (m.get('contenido_neto_total') if m.get('contenido_neto_total') is not None else m.get('unidades'))
            if es_entrada else 0
        ) or 0.0
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for prod, und, pu, pt in zip(productos, unidades, precios, totales):
                ...
                # 🔹 Calculamos el contenido neto total = unidades × cont_neto
                contenido_neto_total = float(und or 0) * cont_neto
                ...
                """, (
                    ...
                    prod.strip(),
                    float(und or 0),
                    contenido_neto_total,  # 👈 Nuevo campo
                    float(pu or 0),
                    float(pt or 0)
                ))
                ...
                if inv:
                    ...
                    """, (float(und or 0), mercancia_id))
                else:
                    cursor.execute("""
                        INSERT INTO inventario (producto, inventario_inicial, entradas, salidas, aprobado, mercancia_id)
                        VALUES (%s, %s, %s, %s, %s, %s)
                    """, (prod, 0, float(und or 0), 0, 0, mercancia_id))

                # 🔹 Aquí ya se multiplica bien
                unidades_reales = float(und or 0) * cont_neto
                ...
                """, (
                    ...
                    mercancia_id,
                    'COMPRA',
                    unidades_reales,   # 👈 ya multiplicado
                    float(pu or 0),
                    numero_factura
                ))
