...
def recetas():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s)
        """, (product_id, material_id, quantity, unit))
        conn.commit()
...
def panel_de_control():
    # -- Reemplaza estas 3 consultas por tus queries reales si ya las tienes --
    conn = conexion_db()
    cur = conn.cursor(dictionary=True)

    # Totales inventario
    cur.execute("""
        SELECT COUNT(*) AS total_items, COALESCE(SUM(disponible),0) AS total_stock
        FROM inventario
    """)
    inventario = cur.fetchone() or {"total_items": 0, "total_stock": 0}

    # Órdenes en proceso (WIP)
    cur.execute("""
    ...
    cur.execute("""
        SELECT COALESCE(SUM(stock),0) AS terminados
